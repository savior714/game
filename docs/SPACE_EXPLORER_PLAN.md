...
- 운영 entry가 열리지 않음
- 사용자가 기존 Space Explorer 흐름을 진행할 수 없는 배포 차단 치명적 회귀
- 데이터 손상
- 보안 또는 개인정보 노출
- 일반 과목 변경이 위 배포 차단 치명적 회귀를 직접 유발함

page error, console error, failed request 또는 정적 자산 이상만으로는 운영 예외가 성립하지 않는다. 이러한 신호가 위 배포 차단 치명적 회귀, 데이터 손상 또는 보안 문제를 직접 증명할 때만 해당 예외 범위에서 조사·수정한다.

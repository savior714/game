...
- **영어 주간 시험 단어 집중 학습 시스템 (2026-03-30)**:
  - 보호자 페이지(`guardian`)에서 주간 시험용 단어(한/영/이모지)를 직접 추가/삭제할 수 있는 관리 UI 구축.
  - 영어 게임 엔진(`english/engine.js`)에 실시간 주간 단어 주입 및 **60% 이상의 빈도**로 출제되는 로직 반영. (레벨 무관, 현재 진행 난이도에 자연스럽게 삽입)
  - `SyncEngine`을 통해 주간 단어 목록 클라우드 동기화 지원.

...
- **보석 경제 시스템 (2026-03-28)**:
  ...
  - **보상바 겹침 (CSS)**: 좁은 폭에서 중앙 pill과 로그아웃이 겹치지 않도록 `inventory-center`에 가로 스크롤·`inventory-rail-right`에 `flex-shrink: 0`·`inventory-auth`에 가변 너비 및 라벨 말줄임 적용(`global/reward.css`).

...
- **로켓 발사 보상 개편 (2026-03-27)**:
  ...
  - **획득 연출**: 20연속 정답(로켓 발사) 시 `playEntranceAndAddGem` 연출(로켓 중앙 폭발 → 보석 +1) 실행.
  - **보석 상점(Shop)**: 보석 💎 1개당 '유튜브 15분', '간식 1개', '마블 게임 1회'로 교환 가능한 상점 모달 구현.
  - **로직 동기화**: `rocket-core.js`에서 보석 획득 연출을 호출하도록 수정하여 전 과목 적용 완료.
  - **검증**: `verify_reward_logic.js`를 통해 보석 충전, 차감, 교환 상태 전이 로직의 무결성 확인.

## Design Decisions

- **Exchange Rate**: 아이의 성취감을 고취하기 위해 모든 보상 단가를 **보석 1개**로 통일 (사용자 피드백 반영).
- **Shop Accessibility**: 별도의 버튼 없이 상단 인벤토리의 보석 구역을 클릭하면 즉시 상점이 열리도록 하여 직관성 확보.
- **Visuals**: 보석 획득 시 큼직한 💎 아이콘과 함께 `+1` 텍스트를 bounce 애니메이션으로 강조.
- **SSOT**: `implementation_plan.md` 및 `task.md`를 통해 보석 시스템 명세 관리.

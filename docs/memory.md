...
- **로켓 발사 보상 연동 복구 (2026-03-27)**:
  - **원인**: `common/rocket-core.js` 공통화 과정에서 보상 룰렛 호출(`RewardSystem.playEntranceAndOpenRoulette`)이 누락되었음을 확인.
  - **해결**: `showBoostBanner()` 내부에서 `RewardSystem` 존재 여부 확인 후 룰렛 표시 함수를 호출하도록 로직 복구.
  - **결과**: 전 과목(수학, 영어, 국어, 과학)에서 20연속 정답(로켓 발사) 시 정상적으로 보상 룰렛이 UI에 표시됨.
- **마블 머지 게임 물리 및 UX 개선 (2026-03-27)**:
  - **물리**: 구슬 반지름의 제곱($r^2$)을 질량으로 사용하는 충격량 공식을 물리 엔진에 도입하여 고레벨 구슬의 무거운 관성을 구현.
  - **판정**: 머지 체크 범위를 실제 반지름 합보다 1.2px 넓게 설정하여 접촉 중 머지 실패 현상을 해결하고, 겹침 해결 계수를 0.51로 상향.
  - **UX**: 상단 미리보기 구슬에 현재 등급 숫자를 렌더링하고, 크기를 17%($0.6r \rightarrow 0.7r$) 확대하여 사용자 예측 편의성 증대.
  - **SSOT**: `CRITICAL_LOGIC.md` 제15조에 물리 최적화 및 판정 규칙 명문화 완료.

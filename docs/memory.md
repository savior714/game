...
- **영어 주간 시험 단어 시스템 개선 (2026-03-31)**:
  - **레이아웃**: `.q-blanked`에 `flex-wrap`을 적용하여 긴 주간 단어("responsibility" 등)가 카드 밖으로 넘치지 않도록 수정.
  - **노출 로직 버그 수정**: 주간 단어 출제 시 최근 10문제 중복 방지 로직을 단어 수에 맞게 동적으로 완화(RECENT_LIMIT 조절)하여 등록된 단어가 적어도 확실히 등장하도록 개선.
  - **문제 유형 다양화**: 동일한 주간 단어 재출제 시 객관식과 스펠링 유형이 번갈아 나오도록 교차 알고리즘 적용.
  - **명시적 로드**: 문제 생성 시 `loadWeeklyWords`를 호출하여 보호자 페이지 설정이 게임에 실시간 반영되도록 동기화 강화.

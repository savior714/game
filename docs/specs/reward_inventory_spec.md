...
- **UX (`guardian/guardian.js`)**: 행 왼쪽 아이콘(이모지) 클릭 시 동일 편집 모달 진입. 아이콘 필드가 비어 있으면 저장 시 기본 이모지로 대체한다. 신규 추가 폼·편집 모달의 아이콘 입력란 옆/아래에 Windows **Win + .** 로 이모지 패널을 열 수 있다는 안내를 둔다.

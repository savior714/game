/**
 * @fileoverview 국어 과목 엔진 - 문제 출제, 정답 평가, 진행률 관리
 * @module korean/engine
 */

/**
 * 국어 문제 데이터
 * @typedef {Array} KoreanQuestion
...
/* ═══════════════════════════════════
   ...
   ※ 실제 데이터는 korean/data/words.json에서 동적 로드됨
══════════════════════════════════ */
...
const STATS_KEY          = ProgressEngine.createStatsKey('korean');
...
function askQuestion() {
  try {
    ...
    if (typeof QuizUICore !== 'undefined' && QuizUICore.handleQuestionError) {
      ...
      console.error('[Korean] Question error:', err);
    }
  }
}

function recordResult(correct, elapsed) {
  const oldLevel = getDifficultyLevel(currentCat);
  ...
  if (correct && typeof DailyStreak !== 'undefined') {
    DailyStreak.recordAnswer(SUBJECT_NAME);
  }
  ...
  // 난이도 상승 감지
  const newLevel = getDifficultyLevel(currentCat);
  if (newLevel > oldLevel && typeof GrowthVisualizer !== 'undefined') {
    GrowthVisualizer.checkLevelUp(SUBJECT_NAME, oldLevel, newLevel);
  }

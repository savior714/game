...
- Asset ID: `scene-seaweed-loop-03-01`
- Runtime alias: `scene.seaweed-loop.03`
- Target canonical path: `domains/ocean-rescue/assets/source/scene/seaweed-loop-03.svg`
- Atlas bundle: `scene`
- Source type: `svg`
...
This asset is the third of three authored seaweed obstacles in the sea-turtle rescue slice of Ocean Rescue.
...
The player must immediately recognize it as a member of the same draggable/releasable seaweed obstacle family as `scene.seaweed-loop.01` and `scene.seaweed-loop.02` sharing the exact same art direction, but as an independently authored third obstacle rather than repeated background scenery.
...
- Placement: `turtleAndObstacle` container on the gameplay plane
  - Positioned and rotated dynamically by runtime according to canonical rope geometry (Rope 3 start `(780, 410)` to end `(1080, 450)`)
...
`scene.seaweed-loop.01` and `scene.seaweed-loop.02` are APPROVED STYLE/FAMILY REFERENCES ONLY.
...
DO NOT copy:
- Path geometry of `scene.seaweed-loop.01` or `scene.seaweed-loop.02`
- Exact outer contour or node positions of loop 01 or loop 02
- Exact internal strand layout or stem branching of previous loops
- Mirrored geometry of loop 01 or loop 02
- Rotation-only or scale-only variants of loop 01 or loop 02

Loop 03 must not be loop 01 or loop 02 with different rotation, different scale, mirroring, path translation, recoloring, or superficial leaf/node relocation.
...
- Visibly different outer contour rhythm from loop 01 and loop 02 (e.g., distinct kelp frond branching, double-node accent, or lower-taper loop rhythm).
...
- Root viewBox: `0 0 120 200`
- Required root group: `scene-seaweed-loop-03`
...
- Visual language and material family of approved `scene.seaweed-loop.01` and `scene.seaweed-loop.02`
...
- Copying or tracing `scene.seaweed-loop.01` or `scene.seaweed-loop.02` geometry
- Mirror, rotate, scale, or recolor variants of previous seaweed loops
...
At the intended in-game size, loop 03 is immediately recognizable as the same seaweed-obstacle family as loop 01 and loop 02 while having independently authored outer contour and internal strand/node geometry that cannot reasonably be described as a mirrored, rotated, scaled, recolored, or trivially edited copy of previous loops.
...
Preserve the required viewBox (`0 0 120 200`), required root group (`scene-seaweed-loop-03`), and transparent background.

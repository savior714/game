(function () {
  ...
  function centerAnchorOnTrimmedVisibleFrame(sprite) {
    ...
    if (
      ...
      throw new Error(
        "Cannot center loop anchor: authored texture.orig is missing or invalid"
      );
    }
    var trim = texture.trim;
    if (trim) {
      if (
        ...
        throw new Error(
          "Cannot center loop anchor: authored texture.trim is invalid"
        );
      }
      sprite.anchor.set(
        (trim.x + trim.width / 2) / orig.width,
  ...
  function syncLoops(current) {
    ...
    for (var i = 0; i < nodes.loops.length; i += 1) {
      ...
      var isDragging = isActive && pointerIntent.active && finite(pointerIntent.x) && finite(pointerIntent.y);

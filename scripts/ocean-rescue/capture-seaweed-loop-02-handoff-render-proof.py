#!/usr/bin/env python3
"""Isolated and in-context render proof harness for scene-seaweed-loop-02 handoff (B23).
...
TASK_ID = "AIDENGAME-OCEAN-RESCUE-SEAWEED-LOOP-02-HANDOFF-RENDER-PROOF"
ASSET_ID = "scene-seaweed-loop-02-01"
...
INBOX_SVG = REPO_ROOT / "domains/ocean-rescue/assets/handoff/inbox/scene-seaweed-loop-02-01.svg"
BRIEF_MD = REPO_ROOT / "domains/ocean-rescue/assets/handoff/briefs/scene-seaweed-loop-02-01.md"
...
def create_in_context_diorama(loop_img_2x: Image.Image) -> Image.Image:
    # 1280x720 diorama composition
    canvas = Image.new("RGBA", (1280, 720), (12, 44, 76, 255))
    draw = ImageDraw.Draw(canvas)
    
    # Deep water gradient backdrop elements
    ...
    # Place seaweed-loop-02 candidate at obstacle position (scale down to 1.2x for diorama placement)
    ...
    # Position 1: Main rescue site obstacle
    canvas.alpha_composite(resized_loop, (740, 320))

    # Position 2: Foreground framing accent
    small_loop = loop_img_2x.resize((int(LOGICAL_SIZE[0] * 0.9), int(LOGICAL_SIZE[1] * 0.9)), Image.Resampling.LANCZOS)
    canvas.alpha_composite(small_loop, (150, 420))

    # Label text overlay
    draw.text((20, 20), "Ocean Rescue — Seaweed Loop 02 In-Context Proof", fill=(255, 255, 255, 230))
    draw.text((20, 45), f"Asset ID: {ASSET_ID} | Alias: {ALIAS}", fill=(180, 220, 240, 200))
    draw.rectangle([735, 315, 745 + target_w, 325 + target_h], outline=(74, 184, 106, 220), width=2)
    draw.text((740, 300), "Candidate obstacle placement", fill=(74, 184, 106, 240))
...
def build_proof() -> int:
    ...
    report_md = f"""# Seaweed Loop 02 Render Proof Report

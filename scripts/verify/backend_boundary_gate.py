#!/usr/bin/env python3
"""BE import graph gate — dependency_boundary_scan + baseline (신규 위반만 FAIL).

Usage:
  python3 scripts/verify/backend_boundary_gate.py
  python3 scripts/verify/backend_boundary_gate.py --check
  python3 scripts/verify/backend_boundary_gate.py --update-baseline
"""
...
BASELINE_PATH = ROOT / "scripts" / "verify" / "backend_boundary_baseline.txt"
...
def collect_violations(target: Path) -> set[str]:
    ...
    backend_only = {
        ...
        if v.file.startswith("src/")
    }
    return backend_only


def main() -> int:
    parser = argparse.ArgumentParser(description="Backend import boundary gate")
    ...
    if not args.target.exists():
        print(f"[be-boundary] SKIP — target missing: {args.target}")
        return 0
    ...
    if args.update_baseline:
        ...
        print(f"[be-boundary] Baseline updated: {len(current)} entries → {BASELINE_PATH}")
        return 0

    new_entries = filter_new_entries(current, loaded)
    print(
        f"[be-boundary] Violations: current={len(current)}, "
        f"baseline={len(loaded)}, new={len(new_entries)}"
    )

    if args.check and new_entries:
        print("[be-boundary] FAIL — new import boundary violations:")
    ...
    if args.check:
        print("[be-boundary] PASS — no new import boundary violations")
    return 0

...
덧셈·뺄셈·곱셈을 8지선다 버튼, 120초 타이머(`TIME_LIMIT`), 세션 10문제로 구성.
...
- **공용 코어**: 난이도·저장·로켓·퀴즈 UI는 `common/progress-engine.js`, `rocket-core.js`, `rocket-effects.js`, `quiz-ui-core.js`에 위임
- **영속성**: `localStorage` (key: `mathGameStats`); 전역 보상은 `study_rewards` (`global/reward.js`)

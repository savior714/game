...
def test_get_diversity_gems_exists() -> None:
    ...
    code = _read()
    assert "function getDiversityGems" in code, "getDiversityGems 함수가 없습니다."
...
def test_1_subject_no_gems() -> None:
    ...
    # 마지막 return 0 (1개일 때)
    assert "return 0" in fn, "subjectCount < 2일 때 0을 반환해야 합니다."
...
def test_2_subjects_gives_1_gem() -> None:
    ...
    fn = _extract_function(code, "function getDiversityGems")
    assert (
        "if (subjectCount >= 2) return 1" in fn or "if(subjectCount>=2)return 1" in fn
    ), "2개일 때 1보석을 반환해야 합니다."
...
def test_3_subjects_gives_2_gems() -> None:
    ...
    fn = _extract_function(code, "function getDiversityGems")
    assert (
        "if (subjectCount >= 3) return 2" in fn or "if(subjectCount>=3)return 2" in fn
    ), "3개일 때 2보석을 반환해야 합니다."
...
def test_4_subjects_gives_2_gems_not_3() -> None:
    ...
    fn = _extract_function(code, "function getDiversityGems")
    assert (
        "if (subjectCount >= 4) return 2" in fn or "if(subjectCount>=4)return 2" in fn
...
def test_today_subjects_exists() -> None:
    ...
    code = _read()
    assert "function getTodaySubjects" in code, "getTodaySubjects 함수가 없습니다."

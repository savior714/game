"""Focused tests for seaweed-loop-02 generated pipeline rebuild (B25)."""
...
def test_seaweed_loop_02_in_atlas_manifest():
    ...
    assert "scene.seaweed-loop.02" in scene_bundle["aliases"]


def test_seaweed_loop_02_in_render_assets_registry():
    assert REGISTRY_JS.exists()
    content = REGISTRY_JS.read_text(encoding="utf-8")
    assert "scene.seaweed-loop.02" in content

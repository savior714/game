...
def test_animation_loop_avoids_unnecessary_idle_renders() -> None:
    ...
    assert "Math.abs(zoomDelta) > SMOOTHING_EPSILON || Math.abs(rotationDelta) > SMOOTHING_EPSILON" in main_js
...
def test_renderer_records_frame_measurements_for_regression_guard() -> None:
    ...
    assert "if (typeof performance !== \"undefined\" && typeof performance.mark === \"function\")" in renderer_js
    assert "performance.mark(PERF_MARK_START);" in renderer_js
    assert "performance.mark(PERF_MARK_END);" in renderer_js
    assert "performance.measure(PERF_MEASURE_NAME, PERF_MARK_START, PERF_MARK_END);" in renderer_js

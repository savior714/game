...
def test_youtube_acknowledged_session_restored_on_bootstrap() -> None:
    server = HTTPServerFixture()
    base_url = server.start()
    domain_url = f"{base_url}/domains/math/index.html"
    ...
    try:
        with sync_playwright() as p:
            ...
            # Pre-seed an acknowledged session (endsAt < now, acknowledgedAt is finite) before bootstrap
            page.evaluate("""() => {
                const now = Date.now();
                const session = {
                    schemaVersion: 1,
                    sessionId: 'yt-ack-seed-456',
                    status: 'acknowledged',
                    startedAt: now - 1000000,
                    endsAt: now - 100000,
                    durationMs: 600000,
